...
class MotionModel:
    def __init__(self,
        ...
        object_uid = unique_id((os.path.basename(dataset_fname), num_params, start_idx, window_size, window_time, lasso,
    ...
    def _read_dataset(self, dataset_fname: str, start_idx: int, num_params: int,
                      pose_model: PoseModel) -> Tuple[np.ndarray, np.ndarray]:
        df = pd.read_hdf(dataset_fname)
        ...
        for begin_segment, end_segment in zip(idx, idx[1:]):
            df_list.append(
                data_ops.series_to_supervised(data_in[begin_segment:end_segment, :],
                                              n_in=self.window_size,
                                              n_step=self.window_time))
        df_list.append(
