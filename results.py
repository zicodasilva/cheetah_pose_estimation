...
def data_driven_analysis(dir_prefix: str):
    ...
    n_comps_vs_mpe = np.array(
        [np.mean(data["mpe"][i : i + comp_block]) for i in range(0, len(data["mpe"]), comp_block)]
    )
    ...
    axd["left"].fill_between(
        n_comps, n_comps_vs_mpe - n_comps_vs_mpe_std, n_comps_vs_mpe + n_comps_vs_mpe_std, alpha=0.1
    )
    axd["left"].set_ylabel("Global MPE (mm)")
    axd["left"].set_xlabel("# Components")
    axd["right"].plot(n_comps, n_comps_vs_mpjpe, marker="o")
    axd["right"].fill_between(
        n_comps, n_comps_vs_mpjpe - n_comps_vs_mpjpe_std, n_comps_vs_mpjpe + n_comps_vs_mpjpe_std, alpha=0.1
    )
    ...
    axd["bottom"].fill_between(
        n_comps, n_comps_vs_time - n_comps_vs_time_std, n_comps_vs_time + n_comps_vs_time_std, alpha=0.1
    )
...
def torque_error(robot1: pe.system.System3D, robot2: pe.system.System3D):
    ...
    for motor in pe.motor.torques(robot1):
        if tau1.shape == ():
            ...
            tau1 = np.concatenate(
                (tau1, pe.utils.get_vals(motor.pyomo_vars["Tc"], (motor.pyomo_sets["Tc_set"],))), axis=1
            )
    for motor in pe.motor.torques(robot2):
        if tau2.shape == ():
            ...
            tau2 = np.concatenate(
                (tau2, pe.utils.get_vals(motor.pyomo_vars["Tc"], (motor.pyomo_sets["Tc_set"],))), axis=1
            )
...
def grf_error(robot: pe.system.System3D, data_path: str, out_dir: str):
    ...
    for foot in pe.foot.feet(robot):
        ...
        if foot.name in foot_contact_order and foot_contact_order[foot.name] is not None:
            ...
            Fz = (
                signal.resample_poly(misc.remove_dc_offset(grf["Fz"].values, 500), up=2, down=35, axis=0)
                * scale_forces_by
            )
...
def _residual_error(points_2d_df, points_3d_dfs, markers, camera_params) -> Dict:
    ...
    for i in range(n_cam):
        ...
        error[str(i)] = (
            pd.concat(error_dfs, ignore_index=True)
            if len(error_dfs) > 0
            else pd.DataFrame(columns=["frame", "marker", "camera_distance", "pixel_residual", "error_u", "error_v"])
        )
...
def _save_error_dists(px_errors, output_dir: str) -> Tuple[float, float, float]:
    ...
    data_ops.save_pickle(
        os.path.join(output_dir, "reprojection.pickle"),
        {
            "error": distances,
